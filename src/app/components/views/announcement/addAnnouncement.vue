<template>
	<div class="base-div">
		<md-card class="dashboard-card">
			<md-card-area>
				<md-card-header>
					<div class="md-title">Add Announcement</div>
				</md-card-header>
				<md-card-content>
					<form>
						<md-input-container>
...
export default {
	beforeMount() {
		this.$store.dispatch("setCurrentRoute", "/addAnnouncement");
	},
	...
	methods: {
		...
		submitadd(e) {
			const { title, message, link } = this.payload;
			...
			if (Object.keys(data).length > 0) {
				this.sendRequest(`/admin/announcements/create`, "POST", null, data, result => {
