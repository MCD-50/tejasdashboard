...
export default {
	...
	mounted() {
		if (this.$store.getters.pageFor == constant.ALERT) {
		...
		getItems(pageNumber = 1, search = null, callback = null) {
			...
			this.sendRequest(`/admin/alerts?page=${pageNumber}&limit=${limit[this.helper.selectedLimitIndex]}${qsearch}`.trim(), 'GET', null, null, (result) => {
				if (result && !result.error && result.value && result.value.result && result.value.result.data) {
					...
					const _data = Object.assign({ pageFor: constant.ALERT }, { data: result.value.result.data, pages: { total:result.value.result.count, current: pageNumber }});
