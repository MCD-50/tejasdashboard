...
export default {
	beforeMount() {
		this.$store.dispatch("setCurrentRoute", "/viewPlans");
	},

	mounted() {
		if (this.$store.getters.pageFor == constant.PLAN) {
	...
	methods: {
		...
		_delete(e, item) {
			this.sendRequest(`/admin/plans/delete/${item._id}`, "DELETE", null, null, result => {
		...
		getItems(pageNumber = 1, search = null, callback = null) {
			...
			this.sendRequest(`/admin/plans?page=${pageNumber}&limit=${limit[this.helper.selectedLimitIndex]}${qsearch}`.trim(), 'GET', null, null, (result) => {
				if (result && !result.error && result.value && result.value.result && result.value.result.data) {
					...
					const _data = Object.assign({ pageFor: constant.PLAN }, { data: result.value.result.data, pages: { total:result.value.result.count, current: pageNumber }});
