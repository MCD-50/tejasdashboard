<template>
	...
	<md-card class="dashboard-card">
			<md-card-area>
				<md-card-header>
					<div class="md-title">View Portfolio</div>
				</md-card-header>
				<md-card-content>
					<form>
						<md-input-container>
						...
						<md-input-container>
							...
							<md-input disabled v-model="payload.quantity"></md-input>
						</md-input-container>
...
export default {
	beforeMount() {
		this.$store.dispatch("setCurrentRoute", "/viewPortfolio");
	},
	mounted() {
		const objectId = this.$route.params.objectId;
		const customerId = this.$route.params.customerId;
		if (objectId && customerId) {
			this.sendRequest(`/admin/portfolios/get/${customerId}/${objectId}`, "GET", null, null, result => {
