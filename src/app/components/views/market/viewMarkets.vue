...
export default {
	...
	mounted() {
		if (this.$store.getters.pageFor == constant.MARKET) {
	...
	methods: {
		...
		view(e, item) {
			this.$router.push({
				path: '/viewMarket', name: 'View Market',
				params: {
		...
		getItems(pageNumber = 1, search = null, callback = null) {
			...
			this.sendRequest(`/admin/markets?page=${pageNumber}&limit=${limit[this.helper.selectedLimitIndex]}${qsearch}`.trim(), 'GET', null, null, (result) => {
				if (result && !result.error && result.value && result.value.result && result.value.result.data) {
					...
					const _data = Object.assign({ pageFor: constant.MARKET }, { data: result.value.result.data, pages: { total:result.value.result.count, current: pageNumber }});
